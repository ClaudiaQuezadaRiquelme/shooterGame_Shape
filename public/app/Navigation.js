...
const navigationTo = (getTo, animationType) => {
    ...
    console.log('navigationTo: getTo & animationType : ', getTo, ' & ', animationType);
    switch (animationType) {
         ...
         */
        case 'fade_in':
            animation_FadeIn(getTo); // Animation.js
            animation_layout.finished.then(() => {
                // Fake splash
                setTimeout(() => {
                    navigationTo('#swiper_page', 'splash_to_menu'); // después de 2 segundos vamos a swiper_page
                }, 2000);
            });
            break;
         ...
         */
        case 'splash_to_menu':
            animation_SplashToMenu(getTo); // Animation.js
            break;
        
        /**
         * Desde menú principal hasta game 
         */
        case 'menu_to_game':
            animation_MenuToMain(getTo); // Animation.js
            break;
        /**
         * Desde game hasta menú principal 
         */
        case 'game_out': // GAME OVER
...
const popUpToggle = (getTo, animationType) => {
    ...
    console.log('popUpToggle: getTo & animationType: ', getTo, ' & ', animationType);
    ...
    switch (animationType) {
        ...
        case 'resume_modal':
            animation_PopupContinue(getTo);
            break;
        case 'confirm_modal_in': // Confirm Quit Game
            animation_ConfirmIn(getTo);
            break;
        case 'confirm_modal_out': // Confirm Quit Game
            animation_ConfirmOut(getTo);
            animation_PopupContinue(getTo);
            break;
